class Product
  def compare_with_url(agent, log, pages, notifications)
    ...
    if item_page.body.include?('dcq_question_subjective_1')
      UserMailer.send_email("Exception in item page: #{item_page}, product: #{attributes.slice(*%w[id ebay_item_id amazon_asin_number])}", 'Exception in compare ebay call', MAILS_TO).deliver
    end
    ...
    if ebay_item[:ack] == 'Failure'
      UserMailer.send_email("Exception in ebay call: #{ebay_item}, product: #{attributes.slice(*%w[id ebay_item_id amazon_asin_number])}", 'Exception in compare ebay call', MAILS_TO).deliver
    else
